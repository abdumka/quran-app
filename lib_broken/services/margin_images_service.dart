  ...
  Future<void> downloadAndEnable() async {
    ...
    if (!downloadUrl.startsWith('http')) {
      throw const FileSystemException(
        'Margin images download URL is not configured',
      );
    }

    final supportDir = await getApplicationSupportDirectory();
    final tempZip = File(p.join(supportDir.path, 'margin_images.zip'));
    ...
    try {
      ...
      if (expectedSha256.isNotEmpty && digest != expectedSha256) {
        debugPrint('SHA-256 mismatch for Margin images: expected $expectedSha256, got $digest');
